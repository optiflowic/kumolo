...
if [[ -z "${KUMOLO_CORS_ALLOW_ORIGIN:-}" ]]; then
  skip "OPTIONS preflight to / returns 200 (KUMOLO_CORS_ALLOW_ORIGIN not set)"
  skip "OPTIONS preflight includes Access-Control-Allow-Origin (KUMOLO_CORS_ALLOW_ORIGIN not set)"
  skip "Actual DynamoDB response includes Access-Control-Allow-Origin (KUMOLO_CORS_ALLOW_ORIGIN not set)"
else
